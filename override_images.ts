...
content = content.replace(/id:\s*'([^']+)'([\s\S]*?)image:\s*'[^']+'([\s\S]*?)images:\s*\[\s*'[^']+'\s*\]/g, (match, id, part1, part2) => {
  const unsplashId = idToImageMap[id];
  if (!unsplashId) return match; // Fallback

  const url = `https://images.unsplash.com/photo-${unsplashId}?auto=format&fit=crop&q=80&w=800`;
  return `id: '${id}'${part1}image: '${url}'${part2}images: [\n      '${url}'\n    ]`;
});
